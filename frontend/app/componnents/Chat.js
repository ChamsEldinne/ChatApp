...
function Chat({displayChat ,setDisplauChat,messages,setMessages ,displayedContact}) {
    ...
    const [pagination ,setPagination]=useState({last_page:1,current_page:0}) ;
    const [currentPage,setCurentPage]=useState(1) ;
    const [reciverUser,setReciverUser]=useState({name:""});
    const [reciverLoading,setReciverLoading]=useState(true) ;
    const [displayButtuns,setDisplyaButtuns]=useState(false)
    const [scrollToBittomn,setScrollToBottomn]=useState(false) ;
    ...
    const fetchMessages=async()=>{
        ...
        try{
            if(!loading){
               ...
               // chatBodyRef.current.scrollTop=chatBodyRef.current.scrollHeight-chatBodyRef.current.innerHeight
                setMessages([...messages,...response.data.messages]) 
                setPagination(response.data.pagination) ;
                setReciverUser(response.data.reciver) ;
                if(currentPage > 1){
    ...
    const handleScroll = async() => {
        if (chatBodyRef.current.scrollTop === 0 && pagination.last_page>currentPage) {
  ...
  return (
    <section className={`${displayChat?"flex ":"hidden"}   z-10 md:flex flex-col flex-auto bg-gray-900 border-l relative border-gray-800`}>
        ...
        <div ref={chatBodyRef} onScroll={()=>{handleScroll()}} className="chat-body scroll-smooth  p-4  flex-1 z-10  overflow-y-scroll " >
            {scrollToBittomn &&  
             <div   onClick={()=>{
                        chatBodyRef.current.scrollTop=chatBodyRef.current.scrollHeight ;
                        setScrollToBottomn(false) ;
               ...
               className='p-2 bg-gray-800 animate-bounce rounded-full bottom-20 left-1/2 -translate-x-1/2 shadow-md z-20 absolute '>
            ...
            {blocks.map((b,index)=><MessageContainer setMessages={setMessages} key={index} block={b} />)}
                ...
                <div className="relative flex-grow">
                    <label>
                        ...
                        className={`${error? "border-red-500":""} rounded-full py-2  pl-3 pr-10 w-full group border border-gray-800 focus:border-gray-700 bg-gray-800 focus:bg-gray-900 focus:outline-none text-gray-200 focus:shadow-md transition duration-300 ease-in`}
                            type="text" value={message} onChange={(e)=>setMessage(e.target.value)} placeholder="Aa"
