...
function Chat({displayChat ,setDisplauChat ,displayedContact}) {
    ...
    const fetchMessages=async()=>{
        try{
            setLoading(true)
            const response = await axiosClient.get(`/api/getMessages?page=${pagination.current_page+1}`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                },
    ...
    useEffect(() => {
        ...
        const currentDiv = chatBodyRef.current;
  ...
  return (
    ...
    <div ref={chatBodyRef} className="chat-body  p-4 flex-1 z-10  overflow-y-scroll " >
        {/* <LoadingSpiner/> */}
        {loading && <ChatLoading />}
        {/* <ChatLoading/> */}

        {blocks.map((b,index)=><MessageContainer  key={index} block={b} />)}
    ...
    <div className="chat-footer flex-none">
        <div className="flex flex-row items-center p-4">
            ...
            <div className="relative flex-grow">
                <label>
                    <input className="rounded-full py-2 pl-3 pr-10 w-full group border border-gray-800 focus:border-gray-700 bg-gray-800 focus:bg-gray-900 focus:outline-none text-gray-200 focus:shadow-md transition duration-300 ease-in"
