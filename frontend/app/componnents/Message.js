function Message({message,prev,next}){
    ...
    <div className={`group flex flex-col  ${message.reciv_or_sent?"items-end":""}`}>
      <div className="flex items-center ">
            <p 
                className={`px-6 py-3 ${prev? "":"rounded-t-full"} ${next? "":"rounded-b-full"} ${message.reciv_or_sent?"rounded-l-full order-last ":" rounded-r-full " }  bg-gray-800 max-w-xs lg:max-w-md text-gray-200`}>
                {message.message}
            </p>
